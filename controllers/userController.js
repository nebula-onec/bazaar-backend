...
exports.homePage = catchAsyncError(async(req, res, next)=>{
  ...
  const orders = await dbQuery(`select order_id, order_date, order_status from user_order where order_status not in (0,3)`, dbName);

  const pendingOrders=orders.length;

  const productInOrders = await dbQuery(`select order_product.*, product.name  from order_product JOIN user JOIN user_order where order_product.product_id=product.product_id and order_product.order_id=user_order.order_id and user_order.order_status!=3`, dbName);
